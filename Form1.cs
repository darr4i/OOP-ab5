      ...
      { "Гантеля", new Bar() },
        ...
        private void pictureBox_MouseUp(object sender, MouseEventArgs e)
        {
            if (e.Button == MouseButtons.Left)
            {
                ...
                {
                    editor.InitPen(Color.Black, Color.Yellow, DashStyle.Solid, (float)brushSizePicker.Value);
        ...
        private void pictureBox_MouseDown(object sender, MouseEventArgs e)
        {
            if (e.Button == MouseButtons.Left)
            {
                if (shapeDict.ContainsKey(Text))
                {
                    ...
                    editor.InitPen(Color.Black, Color.Yellow, DashStyle.Dash, (float)brushSizePicker.Value);
        ...
        bool SelectOrErase(bool toDelete)
        {
            if (Table.RowCount != 0)
            {
                ...
                for (int i = 0; i < Table.RowCount; i++)
                {
                    ...
                    else editor.InitPen(Color.Black, Color.Yellow, DashStyle.Solid, penSize);
